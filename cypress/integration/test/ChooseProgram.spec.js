...
context('Open Choose Program', () => {
  beforeEach(() => {
    ...
    it('Click in Level 1: Full-Stack Developer', () => {
      cy.get('#Choose-Programs').contains('Level 1: Full-Stack Developer').click()
      cy.location().should((location) => {
        expect(location.pathname).to.eq('/us/course/full-stack-web-development-bootcamp-part-time')
      })
    })
    it('Click in Level 2: Software Engineer', () => {
      cy.get('#Choose-Programs').contains('Level 2: Software Engineer').click()
      cy.location().should((location) => {
        expect(location.pathname).to.eq('/us/course/software-engineering-coding-bootcamp')
      })
    })
